...
# This script handles all dependency issues on ARM64
...
install_package() {
    ...
    if pip install "$package" --no-deps; then
        ...
        print_warning "$package failed, trying fallback: $fallback"
        if pip install "$fallback" --no-deps; then
            print_status "$fallback installed successfully"
        else
            print_warning "Both $package and $fallback failed, skipping..."
        fi
    else
        print_warning "$package failed, skipping..."
    fi
}
...
# Install core dependencies one by one with fallbacks
...
if pip install "litellm>=0.1.0" --no-deps; then
    ...
    # Try to install just the core litellm without extras
